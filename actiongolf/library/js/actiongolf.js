...
var i = {
    init: function() {
        ...
        $("#testimonial-carousel").slick({
            infinite: !1
        }).on("setPosition", function(i, s) {
            s.$slides.css("min-height", "1px"),
            s.$slides.find("h3").css("min-height", "1px"),
            s.$slides.css("min-height", s.$slideTrack.height() + "px");
            var e, t = 0;
            s.$slides.each(function() {
                e = $(this).find("h3").outerHeight(!0),
                t < e && (t = e)
            }),
            s.$slides.find("h3").css("min-height", t + "px")
        }),
        $(".download-app").click(function() {
            this.isIOS()
        }
        .bind(this)),
        $(".subscribe-box-text").keyup(function() {
            var i = $(".subscribe-box-text").val();
            this.isValidEmail(i) ? $(".subscribe-box-button").addClass("active") : ($(".subscribe-box-button").removeClass("active"),
        ...
        $(".subscribe-box-button").click(function() {
            ...
            $.ajax({
                ...
                beforeSend: function(i) {
                    ...
                    i.setRequestHeader("Access-Control-Allow-Headers", "Access-Control-Allow-Origin"),
        ...
        .bind(this)),
        $(".close-modal, #modal-shade").click(function() {
            this.toggleModal()
        }
        .bind(this)),
        $(".subscribe-box-text").keyup(function(i) {
            13 === i.keyCode && $(".subscribe-box-button").click()
        }),
        $("body").keyup(function(i) {
            27 === i.keyCode && this.toggleModal()
        }
        .bind(this))
    },
    toggleModal: function(i) {
        i ? ($(".subscribe-error").html(""),
        $(".subscribe-error").css("visibility", "hidden"),
        $(".lds-ellipsis").hide(),
        $(".subscribe-box-button").css("visibility", "visible"),
        $("#success-message").show(),
        $("#modal-shade").show()) : ($("#success-message").hide(),
        $("#modal-shade").hide())
    },
    isIOS: function() {
        return ["iPad Simulator", "iPhone Simulator", "iPod Simulator", "iPad", "iPhone", "iPod", "MacIntel"].includes(navigator.platform) || navigator.userAgent.includes("Mac") && "ontouchend"in document
    },
    isValidEmail: function(i) {
        return /^\b[A-Z0-9._%-]+@[A-Z0-9.-]+\.[A-Z]{2,4}\b$/i.test(i)
    }
};
i.init();
