	...
	$.fn.slideShow = function(options) {
		...
		this.auto = function() {
			...
			if (!slideShow.interval && slideShow.options.interval > 0.001) {
				//alert(slideShow.options.interval);
				slideShow.interval = window.setInterval(function() {
					slideShow.next();
				}, slideShow.options.interval * 1000);
				
				//alert(slideShow.options.interval * 1000);
			}
			// image loading from data-href
			var selectImg = this.find('.slide:eq(' + this.current +')').attr('data-href');
			/*var slideHeight = this.find('.slide').height();*/
			this.find('.slide:eq(' + this.current +')').find('img').attr('src',selectImg).removeClass('align-center');
			/*this.find('.slide:eq(' + this.current +')').find('img').each(function()
			{
				var height = $(this).height();
				var imgTop = (slideHeight-height)/2;
				$(this).css("margin-top",imgTop);
				});*/
			
			
			//this.find('.slide:eq(' + this.current +')').find('img').css("margin-top",imgTop);
		...
		this.gotoSlide = function(index, noanimation) {
			...
			if (index >= this.numSlides) {
				//alert("inedx----"+index+".....Num slides......"+this.numSlides);
				index = 0;
			}
